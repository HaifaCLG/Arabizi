...
def do_main_CS_statistics(structured_data_list,structured_y):
    '''Start with Arabizi and English only'''
    count_CS_no_shared=0
    count_CS_shared=0
    count_shared_sen=0
    count_arabizi_or_english=0
    count_shared_no_CS=0
    count_no_shared_no_Cs=0
    for y in structured_y:
        if('1' in y or '0' in y): count_arabizi_or_english+=1
        if('4' in y and ('0' in y or '1' in y)): count_shared_sen+=1
        if('0' in y and '1' in y and '4' in y):
            #print([i for i in y if i!='5'])
            #print(remove_duplications([i for i in y if i!='5']))
            count_CS_shared+=1
...
if __name__ == '__main__':
    ...
    do_main_CS_statistics(structured_data_list,structured_y)
    ...
    do_main_CS_statistics(structured_X_aut,structured_y_aut)
